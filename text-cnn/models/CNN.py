...
class CNN(nn.Module):
	def __init__(self, batch_size, output_size, in_channels, out_channels, kernel_heights, stride, padding, keep_probab,
				 vocab_size, embedding_length, weights, task_size=1):
		...
		if task_size > 1:  # multi-task with multiple output_size
			assert output_size == list and len(output_size) == task_size
			self.label = []
			for index in range(0, task_size):
				self.label.append(nn.Linear(len(kernel_heights) * out_channels, output_size[index]))
		else:
